...
router.get("/film-compat", requireAuth, async (req, res): Promise<void> => {
  const tenantId = requireTenant(req, res); if (tenantId == null) return;
  const rows = await db.select().from(filmCompatTable)
    .where(eq(filmCompatTable.tenantId, tenantId)).orderBy(asc(filmCompatTable.film));
  res.json(rows);
});
...
router.post("/film-compat/import", requireAdmin, async (req, res): Promise<void> => {
  ...
  await db.transaction(async (tx) => {
    if (importMode === "replace") await tx.delete(filmCompatTable).where(eq(filmCompatTable.tenantId, tenantId));
    await tx.insert(filmCompatTable).values(clean.map((c) => ({ ...c, tenantId })));
  });
...
router.get("/film-compat/sheet", requireAdmin, async (req, res): Promise<void> => {
  const tenantId = requireTenant(req, res); if (tenantId == null) return;
  const [row] = await db.select().from(appSettingsTable)
    .where(and(eq(appSettingsTable.tenantId, tenantId), eq(appSettingsTable.key, SHEET_URL_KEY))).limit(1);
...
router.put("/film-compat/sheet", requireAdmin, async (req, res): Promise<void> => {
  ...
  await db.insert(appSettingsTable).values({ tenantId, key: SHEET_URL_KEY, value: url, updatedAt: new Date() })
    .onConflictDoUpdate({ target: [appSettingsTable.tenantId, appSettingsTable.key], set: { value: url, updatedAt: new Date() } });
  res.json({ url });
});

// Puxa a planilha do Google e SUBSTITUI a tabela de compatibilidade.
router.post("/film-compat/sheet/sync", requireAdmin, async (req, res): Promise<void> => {
  const tenantId = requireTenant(req, res); if (tenantId == null) return;
  const [row] = await db.select().from(appSettingsTable)
    .where(and(eq(appSettingsTable.tenantId, tenantId), eq(appSettingsTable.key, SHEET_URL_KEY))).limit(1);
  ...
  await db.transaction(async (tx) => {
    await tx.delete(filmCompatTable).where(eq(filmCompatTable.tenantId, tenantId));
    await tx.insert(filmCompatTable).values(clean.map((c) => ({ ...c, tenantId })));
  });
...
router.post("/film-compat", requireAdmin, async (req, res): Promise<void> => {
  ...
  const [inserted] = await db.insert(filmCompatTable)
    .values({ tenantId, film: f.slice(0, 120), models: m.slice(0, 1000), notes: (notes ?? "").trim().slice(0, 500) || null })
    .returning();
...
router.patch("/film-compat/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  const [updated] = await db.update(filmCompatTable).set(update)
    .where(and(eq(filmCompatTable.id, id), eq(filmCompatTable.tenantId, tenantId))).returning();
...
router.delete("/film-compat/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  const id = parseInt(String(req.params.id), 10);
  if (isNaN(id)) { res.status(400).json({ error: "ID inválido" }); return; }
  await db.delete(filmCompatTable)
    .where(and(eq(filmCompatTable.id, id), eq(filmCompatTable.tenantId, tenantId)));
  res.json({ ok: true });
});
