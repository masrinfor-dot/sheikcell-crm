...
router.post("/catalog/import/parse", requireAuth, requirePerm("usar_ia"), async (req, res): Promise<void> => {
  ...
  const prompt = [
    `Você organiza listas de fornecedores de celulares E ACESSÓRIOS (mercado brasileiro) em dados estruturados.`,
    `Cada linha ou bloco da lista abaixo descreve um produto: modelo/nome, armazenamento (quando existir), cor(es) e preço de CUSTO (preço do fornecedor pra loja, não o preço de venda ao cliente final).`,
    `IMPORTANTE — a lista pode misturar aparelhos (celulares) com QUALQUER OUTRO produto que uma loja de celulares venda: fone de ouvido, carregador, cabo, capinha, película, power bank, airtag/rastreador, smartwatch, caixa de som, suporte veicular, etc. NÃO ignore nem descarte um item só porque não é um celular — extraia TODO produto da lista, sempre que tiver nome e/ou preço identificável, mesmo sem marca/modelo reconhecido (ex.: "CABO ORIGINAL CHINA TIPO-C 1M" é um produto válido, use o texto da linha como "model" quando não houver um nome de marca mais específico). Pra esses acessórios: "storage"/"ram"/"network" ficam null (não se aplica, não tente adivinhar), "condition" é "novo" a não ser que a lista diga claramente que é usado/seminovo, e "specs"/"characteristics" só preenchidos se você souber algo relevante — do contrário deixe {} e [] em branco, sem forçar campos de celular (processador, câmera, etc.) num acessório. Sugira "categoryPath" apontando pra uma categoria de acessórios (ex.: ["Acessórios"] ou ["Acessórios","Cabos e Carregadores"]) em vez de forçar dentro de "Celulares".`,
    `Ignore emojis, cabeçalhos, informações de garantia/contato/endereço — extraia todo produto de fato (aparelho ou acessório).`,
    ...
    `IMPORTANTE — RAM e rede: alguns modelos (comum em Android, ex.: "Realme Note 70 4/128GB" e "Realme Note 70 8/256GB") têm o MESMO modelo em versões de MEMÓRIA RAM diferentes (ex.: "4/128GB" = 4GB de RAM + 128GB de armazenamento) e/ou tecnologia de rede diferente (4G x 5G), normalmente com preço de custo diferente pra cada versão. Quando a lista indicar RAM (padrão "RAM/ARMAZENAMENTO", ex.: "4/128", "8/256", ou texto explícito "4GB RAM"), preencha "ram" com só o valor da RAM (ex.: "4GB") e "storage" só com o armazenamento (ex.: "128GB") — NUNCA junte os dois em "storage". Quando a lista mencionar "4G" ou "5G" explicitamente pro aparelho, preencha "network" com esse valor.`,
    `IMPORTANTE — RAM e rede QUANDO A LISTA NÃO DIZ (só se aplica a CELULARES — nunca preencha ram/network num acessório): se a lista não mencionar RAM nem rede pra um celular, não deixe os dois em branco só por isso — use seu conhecimento sobre esse modelo específico (mesma lógica de "specs" abaixo) pra preencher com a configuração de fábrica vendida no Brasil (ex.: "iPhone 13" → "ram":"4GB","network":"5G"; "Moto G84" → "ram":"8GB","network":"5G"). Só deixe "ram" e/ou "network" como null quando você genuinamente não souber o valor com confiança, OU quando esse modelo é vendido oficialmente em mais de uma configuração de RAM/rede (aí sem a lista indicar qual, preencher um valor seria adivinhação e criaria uma variante errada). Trate RAM/rede diferentes como variantes DIFERENTES mesmo que o armazenamento seja igual (nunca junte "4/256GB" e "8/256GB" numa variante só).`,
    ``,
    ...
    `Exemplo com RAM/rede INFERIDOS (lista só diz "iPhone 13 128GB Preto R$1700", sem citar RAM/rede — mas o iPhone 13 só existe numa configuração de RAM/rede no Brasil, então preencha com o que você sabe): "variants":[{"storage":"128GB","ram":"4GB","network":"5G","color":"Preto","costPrice":1700}].`,
    ...
    `"condition" deve ser um destes: novo, excelente, muito_bom, bom, outlet. Para CELULARES sem informação clara, use "bom". Use "novo" quando a lista indicar que o aparelho é lacrado/lacrado de fábrica/nunca usado (ex.: "lacrado", "novo", "sealed") — não confunda com "excelente", que é pra seminovo em ótimo estado. Para ACESSÓRIOS (fone, carregador, cabo, capinha, película, airtag, power bank, etc.) sem informação clara, use "novo" — fornecedor de acessório normalmente vende produto novo, não seminovo.`,
