...
import {
  ArrowLeftRight, Plus, X, Trash2, CheckCircle2, Circle, Landmark, Filter, FileSpreadsheet, Pencil, Check,
} from "lucide-react";
...
export default function PagamentosFiliais() {
  ...
  const [editBankForm, setEditBankForm] = useState({ storeId: null as number | null, bankName: "", label: "" });
  ...
  const openEditBank = (b: FinanceBankAccount) => {
    setEditingBankId(b.id);
    setEditBankForm({ storeId: b.storeId, bankName: b.bankName, label: b.label ?? "" });
  };

  const handleSaveBankEdit = async () => {
    if (savingBankEdit || editingBankId == null) return;
    if (!editBankForm.storeId || !editBankForm.bankName.trim()) { toast({ title: "Selecione a filial e informe o banco", variant: "destructive" }); return; }
    setSavingBankEdit(true);
    try {
      const updated = await api.financePayments.bankAccounts.update(editingBankId, {
        storeId: editBankForm.storeId, bankName: editBankForm.bankName.trim(), label: editBankForm.label.trim(),
      });
      setBankAccounts((prev) => prev.map((x) => x.id === editingBankId ? updated : x));
      setEditingBankId(null);
      toast({ title: "Conta bancária atualizada" });
    } catch (err) {
      toast({ title: "Erro ao salvar", description: err instanceof Error ? err.message : "Erro", variant: "destructive" });
    } finally { setSavingBankEdit(false); }
  };

  const handleDeleteBank = async (b: FinanceBankAccount) => {
    if (!window.confirm(`Excluir a conta "${b.bankName}${b.label ? ` (${b.label})` : ""}"?`)) return;
    try {
      await api.financePayments.bankAccounts.remove(b.id);
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="shk-card w-full max-w-md p-6 my-8">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setShowBankForm(false); setEditingBankId(null); }}><X className="w-5 h-5 text-muted-foreground" /></button>
            </div>
            <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
              <div className="rounded-xl border border-border divide-y divide-border">
                {bankAccounts.length === 0 && <p className="text-xs text-muted-foreground p-3">Nenhuma conta cadastrada ainda.</p>}
                {bankAccounts.map((b) => editingBankId === b.id ? (
                  <div key={b.id} className="px-3 py-2 space-y-2">
                    <select value={editBankForm.storeId ?? ""} onChange={(e) => setEditBankForm((f) => ({ ...f, storeId: e.target.value ? Number(e.target.value) : null }))}
                      data-testid={`select-edit-bank-store-${b.id}`} className="w-full px-2 py-1.5 rounded-lg border border-border text-xs">
                      <option value="">Selecione a filial</option>
                      {stores.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                    </select>
                    <input value={editBankForm.bankName} onChange={(e) => setEditBankForm((f) => ({ ...f, bankName: e.target.value }))}
                      placeholder="Nome do banco" data-testid={`input-edit-bank-name-${b.id}`}
                      className="w-full px-2 py-1.5 rounded-lg border border-border text-xs" />
                    <input value={editBankForm.label} onChange={(e) => setEditBankForm((f) => ({ ...f, label: e.target.value }))}
                      placeholder="Apelido (opcional)" data-testid={`input-edit-bank-label-${b.id}`}
                      className="w-full px-2 py-1.5 rounded-lg border border-border text-xs" />
                    <div className="flex gap-2">
                      <button onClick={() => setEditingBankId(null)}
                        className="flex-1 px-2 py-1.5 rounded-lg text-[11px] font-semibold border border-border hover:bg-secondary">
                        Cancelar
                      </button>
                      <button onClick={handleSaveBankEdit} disabled={savingBankEdit} data-testid={`button-save-bank-edit-${b.id}`}
                        className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 rounded-lg text-[11px] font-semibold bg-primary text-white disabled:opacity-40">
                        <Check className="w-3 h-3" /> {savingBankEdit ? "Salvando..." : "Salvar"}
                      </button>
                    </div>
                  </div>
                ) : (
                  <div key={b.id} className="flex items-center justify-between gap-2 px-3 py-2 text-xs">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => api.financePayments.bankAccounts.update(b.id, { isActive: !b.isActive })
                        ...
                        data-testid={`button-toggle-bank-active-${b.id}`}
                      ...
                      <button onClick={() => openEditBank(b)} title="Editar" data-testid={`button-edit-bank-${b.id}`}
                        className="p-1.5 rounded-lg hover:bg-secondary text-muted-foreground">
                        <Pencil className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => handleDeleteBank(b)} title="Excluir" data-testid={`button-delete-bank-${b.id}`}
