...
- The web nginx must NOT set `X-Forwarded-Proto $scheme` when proxying `/api/` — TLS ends at Traefik before nginx, so $scheme is "http" and the API refuses the secure session cookie (login 200 but no Set-Cookie). Pass through `$http_x_forwarded_proto` instead.
