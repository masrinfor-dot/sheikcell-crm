...
- Null-sector fail-open trap (recurs in every new sector-scoped module, e.g. tasks): `t.sectorId === session.userSectorId` is fail-OPEN when both are null — a vendedor with no sector then matches every null-sector record. Always guard `sid != null && t.sectorId === sid` (and `uid != null && ...` for owner checks) so null never equals null.
